<template>
  ...
  <div id="app">
    ...
    <div class="modal-mask" v-if="showlogin">
        ...
        <br>
        <button class="loginbtn" v-on:click="loggedin=true; showlogin=false;">Login!</button>
    </div>
    </div>
	                                    <!-- top/header -->
    <div class="header">
      <div class="navigation">
        <b-button-group>
          ...
          <b-button variant="info" v-on:click="loggedin=false;">Logout</b-button>
		...
		<p class="user">Signed in as {{ input.username }}</p>
      </div>
    <h1> {{ title }} </h1>
	                                    <!-- submit plugin modal -->
